...
var (
	defaultDTOConverter = NewDTOConverter()
)

func GetTypeAlias(typ Typ) []string {
	return defaultDTOConverter.td.GetTypeAlias(typ)
}

func Default(typ string) (v string) {
	return defaultDTOConverter.Default(typ)
}

func Convert(typ, value string) (result interface{}, err error) {
	return defaultDTOConverter.Convert(typ, value)
}

func ConvertReflect(typ, value string, result interface{}) (err error) {
	return defaultDTOConverter.ConvertReflect(typ, value, result)
}
...
func (dc DTOConverter) Convert(typ, value string) (result interface{}, err error) {
	...
	case Date:
		...
		// 	dateTime, err = time.ParseInLocation(parseConfig.timeLayout, value, time.Local)
		// 	if err == nil {
...
func (dc DTOConverter) Default(typ string) (v string) {
	return dc.td.Default(typ)
}
