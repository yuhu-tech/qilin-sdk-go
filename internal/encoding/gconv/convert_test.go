...
import (
	...
	"github.com/yuhu-tech/qilin-sdk-go/internal/encoding/gconv"
)

func Example_GetGenericValue() {
	// use default TypeDirect
	dc := gconv.NewDTOConverter()
...
func Test_TypeDirect(t *testing.T) {
	// direct := gconv.NewTypeDirectWithConfig(false, map[gconv.Typ][]string{})
	// dc := gconv.NewDTOConverter(gconv.WithTypeDirect(direct))
	// use default TypeDirect
	dc := gconv.NewDTOConverter()
	// add some aliases
	gconv.DefaultTypeDirect.AddTypeAliases(gconv.Int, "interage")
...
func Test_GetGenericValue(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bool empty is false", typ: "bool", value: "", wantResult: interface{}(false)},
